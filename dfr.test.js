...
describe("DataFrame Project Tests", () => {
  ...
  test("calculateMean: Should calculate correct average for datasets", () => {
    ...
    expect(calculateMean([[20.5, 21.0]])).toBe(false);  // 2D array not allowed
  });

  test("findTotal: Should calculate correct sum for datasets", () => {
    const salesFigures = [1500.50, 1900.25, "2000.00", "invalid", 1750.75];
    expect(findTotal(salesFigures)).toBe(7151.50);

    const singleSale = [1500.00];
    expect(findTotal(singleSale)).toBe(1500.00);

    // Invalid inputs
    expect(findTotal([[1500.50]])).toBe(false);  // 2D array not allowed
  ...
  test("flatten: Should convert single-column DataFrame to Dataset", () => {
    ...
    expect(flatten(monthlyTemperatures)).toEqual([20.5, 21.0, 22.5, 19.8, 20.2]);
